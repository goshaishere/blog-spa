...
export const getRecord = (id_record, partnership_id) => {

    let getRecordURL = `https://api.skilla.ru/mango/getRecord?record=${id_record}&partnership_id=${partnership_id}`
    //let params = `?record=${id_record}&partnership_id=${partnership_id}`
    return async (dispatch) => {
        try {
            dispatch(setIsFetching(true))
...
export const getCalls = (date_start, date_end, in_out) => {
    ...
    //let ourUrl = `https://api.skilla.ru/mango/getList?date_start=${'2020-01-01'}&date_end=${'2020-12-02'}`
    if (in_out === 1 ){
